...
goreport: goreport_dep ## Make goreport
	...
	@./graphql_harp_badge/update_goreport_grade.sh

build: ## Build the binary file
	@$(GOROOT)/bin/go build -o $(PROJECT_NAME) main.go

docker: ## Build docker image and push it to private docker registry
	@sudo docker build -t graphql_harp .
	@sudo docker tag graphql_harp:latest 192.168.110.230:5000/graphql_harp:latest
	@sudo docker push 192.168.110.230:5000/graphql_harp:latest
